...
- GET \`/post\` 对应 \`SELECT * FROM posts\`，返回动态列表
- POST \`/post\` 在 body 带 token、content，\`getUserId\` 后 INSERT
...
- GET /wrong 全量调试
- GET /wrong/:token 按 token 解 userId 后 WHERE userid = ?
- POST /wrong 答题错误时写入题目快照
