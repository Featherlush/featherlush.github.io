...
- 只认 ⇒ 行，前提区 AG_1 ⟂ AB 不进列表：截掉 [ 后的机器备注，列表里只留人类可读结论

### 播放与手动控制

setInterval 按 speed 递增 currentIndex，到头循环。滚轮在结论区可上下切换并自动暂停播放。点击某行调用 setCurrentIndex 并做边界钳制。
...
- 共圆检测、点在多边形内等可用库函数减少手写：若把 Python SymPy 管线迁到前端，geometric 可作轻量替代
...
resize 监听父容器，每帧 clearRect 全量重绘。watch deep 监听高亮对象，点数少于 15 时足够；上百点需改脏矩形或分层 Canvas。
