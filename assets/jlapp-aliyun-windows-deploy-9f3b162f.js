...
1. 阿里云控制台创建云服务器 ECS，系统选 Windows Server
2. 记下公网 IP，仅自己保存，不要写进公开仓库
3. 安全组先放行远程桌面 RDP，以及后端应用监听端口
4. 本机远程桌面连接，输入公网 IP 与管理员账号登录
...
- 改数据库连接、jwtSecret 等配置：终端里 npm install、node app.js 试跑：服务器本机浏览器访问本地端口或调试路由，确认能查用户表

## 3. 把代码弄到服务器上

常见几种方式我混用过：

- Git 克隆：服务器装 Git 后 clone 私有仓库凭据勿泄露：压缩包：本机打包 backend 排除 node_modules，远程桌面里解压，再 npm install：VS Code Remote 若后来配置：本地连远程开发；当时主要 RDP 与服务器上的 VS Code
...
- 测试阶段：PowerShell 窗口开着 node app.js：演示前：可用 pm2 for Windows 或任务计划程序保活：确认崩溃后日志写到哪里 console.error 在终端里直接看
...
- 静态资源路径见 HBuilder 篇，Vite base 为相对路径，与 API 地址无关
- API 地址必须是手机可访问的 HTTP 或 HTTPS 服务地址，不能用相对路径代替
