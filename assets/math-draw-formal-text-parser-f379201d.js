...
- 第一行：题目 id，如 translated_imo_2000_p1：第二行：分号分隔的构造命令，真正驱动摆点的部分：分隔线后：定理前提人类可读与 ⇒ 结论，给右侧滚动列表用
...
- 跳过首行标题，把剩余行拼成一条再按 ; 切，避免换行把命令截断：每个片段形如 m = on_circle m g1 a, on_circle m g2 b，逗号表示同一对象的多个约束
...
- 构造命令到约束列表，再到数值求解或 WASM 调库：第一遍仍建议 AST 式元素表，与坐标求解分离
