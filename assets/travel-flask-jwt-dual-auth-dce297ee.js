...
## admin_or_token_required 装饰器

一套装饰器兼容两种调用。部分接口既要移动端用户访问，又要管理员密钥 bypass，我写了一个组合装饰器：
...
## token_required 与 jwt_required 别混用

项目里还有一层自定义 token_required，内部调 jwt_required 再把 user_id 挂到 request.user_id：
...
- 移动端用户接口：统一 jwt_required 与 int get_jwt_identity：管理 CRUD：只验 X-Admin-Key：少数两者都要：用 admin_or_token_required
