...
- payload 只放 userId，不放用户名、密码：jwtSecret 必须来自单一配置文件或环境变量，全项目共用一个来源：竞赛里 \`expiresIn\` 设得很长是为了演示不断线；正式环境应短过期并配合刷新 token。
...
- 登录后 \`localStorage.setItem('Authorization', token)\`，并与 Vuex \`changeLogin\` 同步：或拦截器改为读 localStorage.getItem token
...
## choose 与 changekey 等旁路

- POST /choose 登录后写身份年级，与 JWT 并行，不替代登录
- changekey 与 checkkey 改本地密钥习惯，别和 JWT secret 混为一谈

## 安全备忘

- 生产用 HTTPS，secret 与数据库密码进环境变量，不进 Git
- 缩短 token 有效期，敏感列表接口按 userId 过滤
- 云服务器上 secret 单独配置，与本地 config.js 分离
