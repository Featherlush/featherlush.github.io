...
- 名称与后端 database 配置一致项目里为 jlapp
- 字符集选 utf8mb4，排序规则 utf8mb4_unicode_ci 或默认 utf8mb4，避免中文昵称、帖子内容乱码
- 不要用已废弃的 utf8 三字集 emoji 和部分汉字会出问题
...
1. 执行建表 SQL 文件：Workbench 菜单 File 选 Run SQL Script，或打开 \`.sql\` 粘贴执行
   - 适合一次性初始化 users、posts、answer、wrong、ranking 等全套表
...
- 1～2 个测试用户已知用户名密码，密码走 bcrypt 入库，不要手改哈希列
- 少量帖子、题目行，保证首页、讨论区、闯关不全空
- 年级或题库筛选用的 source、name 与前端 filter 字符串完全一致，如三年级
...
Workbench：Server 菜单 Data Export
...
- 为应用建专用账号如 jlapp_app，只授 jlapp 库上的 SELECT, INSERT, UPDATE, DELETE
- root 仅本机维护用
- 不要在阿里云安全组对外开放 3306；MySQL 只给同机 Node 或内网访问
