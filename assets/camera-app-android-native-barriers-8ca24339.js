...
- 与拍照瞬间闪光的控制路径不同：和 Canvas 预览同时占用同一条 MediaStreamTrack 时，个别 ROM 会重启相机流，AR 链路中断
...
### 障碍 4：反射调 SensorManager

理论上可用 plus.android.importClass android.hardware.SensorManager 注册 SensorEventListener 读陀螺仪，但这要求：

- 自定义 5+ 扩展或 unsafe 反射，无 TypeScript 保障：与 JS 主线程回调、Web AR 的 rAF 同步自己处理：应用商店与隐私合规要单独说明传感器数据用途
...
## DeviceOrientation 与 DeviceMotion

用陀螺仪补偿贴纸抖动是 AR 稳像的常见思路，但在本项目里最终落空。浏览器标准是：
...
iOS 需用户手势授权，Android Chrome 亦逐步收紧。很多环境下默认不派发或需 HTTPS 与权限弹窗，与打开页面即开相机的体验冲突。
